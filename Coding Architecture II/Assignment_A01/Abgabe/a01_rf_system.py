...
class RFSystem:
    ...
    def centerlines(self) -> list:
        return [
            self.mesh.edge_attribute(edge, "centerline") for edge in self.mesh.edges()
        ]
    ...
    def _trim_centerline_at_boundary(
        self, edge, centerline, next_edge, prev_edge
    ) -> None:
        ...
        if boundary_edge is not None:
            intersection = intersection_segment_segment(
                centerline, self.mesh.edge_line(boundary_edge)
            )[0]

            if centerline.end.distance_to_point(
                intersection
            ) < centerline.start.distance_to_point(intersection):
