...
#     def __init__(
#         self, u_count: int, v_count: int, brep: rg.Brep, full_quads: bool = False
#     ):
...
#                 if self.full_quads and len(vertices) != 4:
#                     continue
...
class HexaMesher(BaseMesher):
    ...
    def generate_mesh(self) -> Mesh:
        ...
        tri_mesher = TriMesher(
            self.u_count * 2, self.v_count * 2, self.brep, self.closed
        )
        ...
        for fk in tri_mesh.faces():
            coords = [
                tri_mesh.vertex_coordinates(v) for v in tri_mesh.face_vertices(fk)
            ]
        ...
        for vk in tri_mesh.vertices():
            ...
            adj_faces = [
                fk for fk in adj_faces if fk is not None and fk in face_to_centroid
            ]
            if len(adj_faces) < 3:
                continue
